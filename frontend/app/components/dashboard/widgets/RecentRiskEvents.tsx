...
export default function RecentRiskEvents({
  ...
  const alertEvent = previewRows[0];
  ...
  return (
    <>
      {showAlertToast && alertEvent ? (
        <div className="fixed right-6 top-6 z-50 w-[420px] max-w-[calc(100vw-48px)] rounded-xl border border-rose-200 bg-white p-4 shadow-xl shadow-slate-200">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-base font-bold text-slate-950">
                {alertEvent[0]} · {alertEvent[4]}
              </p>

              <p className="mt-1 text-xs leading-5 text-slate-500">
                {alertEvent[2]}에서 {alertEvent[3]}로 향한 이벤트가 우선 확인
                대상으로 분류되었습니다.
              </p>
            </div>
            ...
            <button
              ...
              className="shrink-0 whitespace-nowrap rounded-md px-2 py-1 text-xs text-slate-400 transition hover:bg-slate-100 hover:text-slate-600"
            >
              닫기
            </button>
          </div>
          ...
          <div className="mt-3 flex gap-2">
            <button
              ...
              onClick={() => {
                ...
                setShowAlertToast(false);
                onMoveTab?.("reports", getReportFilters(alertEvent));
              }}
              className="h-9 flex-1 rounded-md bg-rose-600 px-3 text-xs font-semibold text-white transition hover:bg-rose-700"
            >
              AI 리포트 보기
            </button>
            ...
            <button
              ...
              className="h-9 rounded-md border border-slate-200 px-3 text-xs font-medium text-slate-700 transition hover:bg-slate-50"
            >
              대시보드에서 확인
            </button>
          </div>
      ...
      <Panel title="우선 확인 이벤트" action={`${previewRows.length}건 미리보기`}>
        <div className="mb-4 rounded-md border border-slate-200 bg-slate-50 px-4 py-3">
          <p className="text-xs leading-5 text-slate-500">
            위험도, 상태, 공격 유형, 예측 신뢰도를 종합해 관리자가 먼저 확인해야 할
            이벤트를 정렬합니다. 이벤트를 선택한 뒤 AI 대응 리포트로 바로 이동할 수
            있습니다.
          </p>
        </div>
            ...
            <table className="w-full min-w-[780px] border-separate border-spacing-0 text-left text-sm">
              ...
              <tbody>
                {previewRows.map((row) => {
                  ...
                  const isTopPriority = previewRows[0]?.[0] === row[0];
